...
import { Home, Footer, About, Products, ProductDisplay, SearchResults, Cart, Checkout } from './components';
...
function App() {
  ...
  //import product list from commerce.js
  ...
  const Navbar = () => {
    ...
    let cartQuantity = cart.total_items;
    ...
    return (
      ...
      <div className="navigation">
        ...
        <div className="buttons">
          ...
          <Link to='/cart' className='button-child'>
            ...
            <sup id="cart-quantity">{ cart.total_items > 0 ? cartQuantity : '' }</sup>
          </Link>
  ...
  //Remove item with the 'x' on cart page
  ...
  useEffect(() => {
    fetchProducts(); fetchTees(); fetchPullOvers(); fetchSweats(); setSearchResults(); fetchCart(); fetchSubdivisions()
  }, []);
    ...
    <Router>
        ...
        <div className='body'>
          <Routes>
            ...
            <Route path='/item' element={<ProductDisplay onAddToCart={handleAddToCart} />}/>
            <Route exact path="/cart" element={<Cart cart={cart} onRemoveFromCart={handleRemoveFromCart}/>} />
            <Route exact path="/checkout" element={<Checkout cart={cart} subdivisions={subdivisions} order={order} onCaptureCheckout={handleCaptureCheckout} error={errorMessage} />} />
          </Routes>
        </div>
        <Footer />
    </Router>
